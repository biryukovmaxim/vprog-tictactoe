 ...
 * @returns {Uint8Array}
 */
export function claim_tx(covenant_id_hex, permission_txid_hex, permission_index, permission_rent, old_root_hex, old_unclaimed, depth, leaf_index, leaf_spk_hex, leaf_amount, new_root_hex, new_unclaimed, siblings_hex, delegate_utxos, fee) {
    const ptr0 = passStringToWasm0(covenant_id_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len0 = WASM_VECTOR_LEN;
    const ptr1 = passStringToWasm0(permission_txid_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len1 = WASM_VECTOR_LEN;
    const ptr2 = passStringToWasm0(old_root_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len2 = WASM_VECTOR_LEN;
    const ptr3 = passStringToWasm0(leaf_spk_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len3 = WASM_VECTOR_LEN;
    const ptr4 = passStringToWasm0(new_root_hex, wasm.__wbindgen_malloc, wasm.__wbindgen_realloc);
    const len4 = WASM_VECTOR_LEN;
    const ptr5 = passArrayJsValueToWasm0(siblings_hex, wasm.__wbindgen_malloc);
    const len5 = WASM_VECTOR_LEN;
    const ptr6 = passArrayJsValueToWasm0(delegate_utxos, wasm.__wbindgen_malloc);
    const len6 = WASM_VECTOR_LEN;
    const ret = wasm.claim_tx(ptr0, len0, ptr1, len1, permission_index, permission_rent, ptr2, len2, old_unclaimed, depth, leaf_index, ptr3, len3, leaf_amount, ptr4, len4, new_unclaimed, ptr5, len5, ptr6, len6, fee);
    if (ret[3]) {
        throw takeFromExternrefTable0(ret[2]);
    }
...
async function __wbg_init(module_or_path) {
    ...
    if (typeof module_or_path === 'undefined') {
        module_or_path = new URL('vprog_tictactoe_encoder_wasm_bg.wasm', import.meta.url);
    }
    const imports = __wbg_get_imports();
