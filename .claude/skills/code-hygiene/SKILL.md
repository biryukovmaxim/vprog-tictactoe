...
Write continuation text as a single long line. `just fmt` (rustfmt
...
5. **After edits**: run `just check` and report any failures.
6. **Format**: keep the diff tight. Don't reflow code or doc paragraphs
   ...
   re-wrapping). Format via `just fmt`; never invoke
   `cargo +nightly fmt` directly: the justfile recipe covers the
   workspace and the excluded guest crate.
