...
//! body bytes are what `Lock::encode` writes (tag-less). The multisig body rules live in the
//! battery's decoder (`MultisigLockView::decode`); this validator delegates to it and requires it
//! to consume the body exactly, so the rule set is never duplicated here.

use vprogs_zk_backend_risc0_runtime_processor::lock_trait::Lock;

use crate::runtime::lock::{LockEnum, MultisigLockView, SchnorrLockView, UnlockedLockView};
...
pub fn validate_lock_body(tag: u8, body: &[u8]) -> Result<(), &'static str> {
    match tag {
        ...
        MultisigLockView::TAG => {
            let mut buf = body;
            MultisigLockView::decode(&mut buf).map_err(|_| "lock.multisig: invalid body")?;
            if !buf.is_empty() {
                return Err("lock.multisig: body has trailing bytes");
            }
            Ok(())
        }
...
mod tests {
    ...
    /// Trailing bytes after a well-formed multisig body are rejected (the `n_pubkeys`-implied
    /// length must consume the body exactly).
    #[test]
    fn decode_lock_body_rejects_trailing_bytes() {
        fn key(b: u8) -> [u8; 32] {
            [b; 32]
        }
        let mut body = Vec::new();
        body.extend_from_slice(&[1u8, 1]);
        body.extend_from_slice(&key(1));
        body.push(0); // one byte too many
        assert!(decode_lock_body(MultisigLockView::TAG, &body).is_err());
    }
}
