//! Lock enum + dispatch: which lock kinds this program accepts.
//!
//! The variant structs and their `Lock` impls are imported from the battery's
//! `lock_variants` (and re-exported below); this file defines the app's `LockEnum` over them and
//! the dispatch: tag byte → variant, and variant → the right `AuthContext` bucket at unlock time.
...
#[cfg(test)]
mod tests {
    use vprogs_zk_backend_risc0_runtime_processor::auth_context::{
    ...
    fn id_hash_matches_sha256_with_domain_of_tag_and_body() {
        ...
        // The lock-identity prefix is battery-owned (`Lock::id_hash`).
        use vprogs_zk_backend_risc0_runtime_processor::domain::Domain;
